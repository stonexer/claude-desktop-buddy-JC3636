// 5x7 bitmap font (rendered into a 6x8 cell with trailing space column
// and blank 8th row for line spacing). Adapted from Adafruit_GFX's
// glcdfont.c — Apache 2.0. Each character is 5 consecutive bytes, each
// byte representing a vertical column (bit 0 = top row, bit 6 = row 7).
//
// Array is indexed by unsigned byte; the full 0..255 range is present so
// we don't need to bounds-check anywhere.
